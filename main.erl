...
%c(main), c(calculate_flow), c(survivor), c(fluidumTyp), c(fluidumInst), c(pipeTyp),	c(pipeInst),	c(resource_type), c(resource_instance),	c(msg),	c(location), c(connector), c(pumpTyp), c(pumpInst), c(flowMeterTyp), c(flowMeterInst).
%main:start({0, 10}).

%tests:
%eqc:quickcheck(main:prop_ordered()).
%eunit:test(main).
%typeEr output zit in typeEr.txt
...
start(Parameter) ->
  ...
  %Onderstaande 2 lijnen commenten als je quickcheck of eunit-test probeert uit te voeren
  %,
  ...
  test_ten() ->
    ...
    %alle 100 testen zijn succesvol indien bereik van 1 naar 10 gaat, als 0 - 1 in het bereik zit komt de flow negatief uit en faalt de test volgens de voorwaarden.
